...
# Each setting is applied as its own cctk call so a single bad value doesn't fail
# the rest. Password flags are appended to every call when present.
...
foreach ($canonical in @(
    ...
    $desc = Get-DellBiosSetting -CanonicalName $canonical
    if (-not $desc) {
