...
## $CustomFieldGoogleChromeActiveBoolean - Name of the NinjaRMM boolean (1/0) custom field for THIS script's result (default: "RemoteUser")
## $CustomFieldGoogleChromeContextString - Name of the NinjaRMM text custom field shared with the system-context script (default: "RemoteContext")
...
if ($RMM -eq 1) {
    ...
    Update-CRDContextField -FieldName $CustomFieldGoogleChromeContextString -ThisContext "User" -IsActive ([bool]$isActive)
} else {
    ...
    Write-Host "Would have updated context field '$CustomFieldGoogleChromeContextString' with label 'User' (active = $isActive)"
}
