...
## $PasswordLength / $env:PasswordLength - Default password length (default: 16)
## $localUser      / $env:localUser      - Default local user name (default: "admin")
...
# either $PasswordLength/$localUser or their $env: form, whichever the input arrived in.
...
function Add-UserToLocalAdministrators {
    ...
    try {
        ...
        Write-Host "Note: User may already be in Administrators group or another error occurred: $($_.Exception.Message)"
    }
}
